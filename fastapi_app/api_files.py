...
router = APIRouter(prefix="/api/files", tags=["Explorateur de fichiers"])
...
def list_files(
    base: str = Query(
        default="",
        description="Sous-dossier à explorer (ex: 'memory' ou 'fastapi_app')",
    ),
    max_depth: int = Query(
        default=4,
        ge=1,
        le=10,
        description="Profondeur maximale de recherche (par défaut 4)",
    ),
):
    """
    ...
    if not base_path.exists():
        raise HTTPException(
            status_code=404, detail=f"Répertoire introuvable : {base_path}"
        )
    ...
    return {"base": str(base_path), "count": len(files), "files": files}
